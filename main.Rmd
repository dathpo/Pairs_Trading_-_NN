...
```
```{r, fig.width=11, fig.height=4}
ggplot(data=melted.div.train.prices.df, aes(x=as.Date(melted.div.train.prices.df$date), y=melted.div.train.prices.df$value,
                           group=melted.div.train.prices.df$variable, color=melted.div.train.prices.df$variable)) +
geom_line() +
xlab("Date") + ylab("Return") + scale_y_continuous(labels=percent) +
...
eem.gld.train.spread <- spread(pair.train.prices.df$EEM, pair.train.prices.df$GLD)
gld.eem.train.spread <- spread(pair.train.prices.df$GLD, pair.train.prices.df$EEM)
pair.train.prices.df <- data.frame(train.prices.df$date, pair.train.prices.df$EEM, pair.train.prices.df$GLD)
pair.train.prices.spread.df <- data.frame(train.prices.df$date, gld.eem.train.spread)
colnames(pair.train.prices.df) <- c("date", "EEM", "GLD")
colnames(pair.train.prices.spread.df) <- c("date", "S(GLD, EEM)")
# pair.pos <- spread(div.train.prices.df$GLD, div.train.prices.df$EEM) >= hist.mean(div.train.prices.df$GLD, div.train.prices.df$EEM) + (0.0*hist.std(div.train.prices.df$GLD, div.train.prices.df$EEM))
...
# train.eem.long <- train.eem.long * 0.5
# train.eem.long <- train.eem.long - 0.25
# train.gld.long <- train.gld.long * 0.5
# train.gld.long <- train.gld.long - 0.25
...
final.df <- data.frame(train.prices.df$date, pair.train.prices.df$EEM, pair.train.prices.df$GLD, gld.eem.train.spread, train.eem.long, train.gld.long)
colnames(final.df) <- c("date", "EEM", "GLD", "spread", "Long_EEM", "Long_GLD")
# colnames(final.df) <- c("date", "EEM", "GLD", "S(GLD, EEM)", "Long EEM", "Long GLD")
melted.final <- melt(final.df, id="date")
gathered <- gather(final.df, key="measure", value="value", c("EEM", "GLD", "spread", "Long_EEM", "Long_GLD"))

melted.train.pair <- melt(pair.train.prices.df, id="date")
melted.train.pair.pos <- melt(pair.train.prices.pos.df, id="date")
```
```{r, fig.width=11, fig.height=4}
melted.final$var <- ifelse(melted.final$variable == "EEM" | melted.final$variable == "GLD", "ETFs", ifelse(melted.final$variable == "spread", "Spread", "Position"))

ggplot(data=melted.final, aes(x=as.Date(date), y=value, group=variable, color=variable)) + geom_line() + xlab("Date") + ylab("Return") + scale_y_continuous(labels=percent) + scale_color_discrete("ETF") + ggtitle("Performance of EEM and GLD ETFs over Training Period (Jan 2011 - Dec 2016) - Normalised") + facet_grid(var~., scale='free_y')
```
```{r, fig.width=11, fig.height=4}
plot.pair <- ggplot(data=melted.train.pair, aes(x=as.Date(melted.train.pair$date), y=melted.train.pair$value, group=melted.train.pair$variable, color=melted.train.pair$variable)) + geom_line() + xlab("Date") + ylab("Return") + scale_y_continuous(labels=percent) + scale_color_discrete("ETF") + ggtitle("Performance of EEM and GLD ETFs over Training Period (Jan 2011 - Dec 2016) - Normalised")

plot.spread <- ggplot(data=pair.train.prices.spread.df, aes(x=as.Date(pair.train.prices.spread.df$date), y=pair.train.prices.spread.df$`S(GLD, EEM)`)) + geom_line() + xlab("Date") + ylab("Distance") + scale_y_continuous(labels=percent) + scale_color_discrete("ETF") + ggtitle("Spread between EEM and GLD ETFs")

plot.pos <- ggplot(data=melted.train.pair.pos, aes(x=as.Date(melted.train.pair.pos$date), y=melted.train.pair.pos$value, group=melted.train.pair.pos$variable, color=melted.train.pair.pos$variable)) + geom_line() + xlab("Date") + ylab("Position") + scale_y_continuous(labels=percent) + scale_color_discrete("ETF") + ggtitle("Performance of EEM and GLD ETFs over Training Period (Jan 2011 - Dec 2016) - Normalised")

plot_grid(plot.pair, plot.spread, plot.pos, labels = "AUTO", ncol = 1, align = 'v')
