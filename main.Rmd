...
gathered <- gather(final.df, key="measure", value="value", c("EEM", "GLD", "S(GLD, EEM)", "Long EEM", "Long GLD"))

melted.train.pair <- melt(pair.train.prices.df, id="date")
melted.train.pair.pos <- melt(pair.train.prices.pos.df, id="date")
```
```{r, fig.width=11, fig.height=4}
melted.final$var <- ifelse(melted.final$variable == "EEM" | melted.final$variable == "GLD", "ETF Pair", ifelse(melted.final$variable == "S(GLD, EEM)", "Pair Spread", "Position"))

ggplot(data=melted.final, aes(x=as.Date(date), y=value, group=variable, color=variable)) + geom_line() + xlab("Date") + ylab("") + scale_y_continuous(labels=percent) + scale_color_discrete("Legend") + ggtitle("ETF Pair EEM-GLD Performance, Spread and Trading Positions over Training Period") + facet_grid(var~., scale='free_y')
```
```{r, fig.width=11, fig.height=4}
